...
int lcmt_viewer_geometry_data::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    ...
    tlen = __string_encode_array(
        buf, offset + pos, maxlen - pos, &string_data_cstr, 1);
    if(tlen < 0) return tlen; else pos += tlen;
...
int lcmt_viewer_geometry_data::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    ...
    tlen = __int32_t_decode_array(
        buf, offset + pos, maxlen - pos, &__string_data_len__, 1);
    if(tlen < 0) return tlen; else pos += tlen;
    if(__string_data_len__ > maxlen - pos) return -1;
    this->string_data.assign(
        static_cast<const char*>(buf) + offset + pos, __string_data_len__ - 1);
