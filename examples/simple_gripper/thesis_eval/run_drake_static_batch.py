#!/usr/bin/env python3
"""Batch runner for DrakeStatic grasp simulation.
...
def command_for_grasp(
    ...
    uogp_object: Optional[str],
    advance_sim_to: Optional[float],
) -> List[str]:
    pos = grasp["position"]
    ...
    cmd = [
        ...
        f"--manual_correction={manual_correction}",
        f"--table_correction={table_correction}",
    ]
    if not use_height_correction:
        cmd.append("--NoHeightCorrection")
    if uogp_object:
        cmd.append(f"--uogp_object={uogp_object}")
    if advance_sim_to is not None:
        cmd.append(f"--advanceSimTo={advance_sim_to}")
    return cmd
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Batch-run DrakeStatic for grasp candidates.")
    parser.add_argument("--input_csv", type=Path, required=True, help="Input grasp CSV (semicolon-separated).")
    parser.add_argument(
        "--output_csv",
        type=Path,
        default=None,
        help="Output CSV path. Default: <input>_withDrakeResults.csv",
    )
    parser.add_argument(
        "--binary",
        type=Path,
        default=default_binary_path("DrakeStatic"),
        help="Path to DrakeStatic binary (default: bazel-bin/examples/simple_gripper/DrakeStatic).",
    )
    ...
    parser.add_argument("--advance_sim_to", type=float, default=None)
    ...
    if args.output_csv is None:
        args.output_csv = args.input_csv.with_name(f"{args.input_csv.stem}_withDrakeResults{args.input_csv.suffix}")
    ...
    for i, grasp in enumerate(grasps, start=1):
        cmd = command_for_grasp(
            binary=args.binary,
            grasp=grasp,
            manual_correction=args.manual_correction,
            table_correction=table_correction,
            use_height_correction=args.use_height_correction,
            uogp_object=args.uogp_object,
